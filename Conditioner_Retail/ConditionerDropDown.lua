...
function CONDITIONERDROPDOWNMENUDelegate_OnAttributeChanged(self, attribute, value)
    if (attribute == "createframes" and value == true) then
        CONDITIONERDROPDOWNMENU_CreateFrames(
            self:GetAttribute("createframes-level"),
            self:GetAttribute("createframes-index")
        )
...
function CONDITIONERDROPDOWNMENU_CreateFrames(level, index)
    while (level > CONDITIONERDROPDOWNMENU_MAXLEVELS) do
        ...
        local newList =
            CreateFrame(
            "Button",
            "ConditionerDropDownList" .. CONDITIONERDROPDOWNMENU_MAXLEVELS,
            nil,
        ...
        for i = CONDITIONERDROPDOWNMENU_MINBUTTONS + 1, CONDITIONERDROPDOWNMENU_MAXBUTTONS do
            local newButton =
                CreateFrame(
                "Button",
                "ConditionerDropDownList" .. CONDITIONERDROPDOWNMENU_MAXLEVELS .. "Button" .. i,
                newList,
    ...
    while (index > CONDITIONERDROPDOWNMENU_MAXBUTTONS) do
        ...
        for i = 1, CONDITIONERDROPDOWNMENU_MAXLEVELS do
            local newButton =
                CreateFrame(
                "Button",
                "ConditionerDropDownList" .. i .. "Button" .. CONDITIONERDROPDOWNMENU_MAXBUTTONS,
                _G["ConditionerDropDownList" .. i],
                "UIDropDownMenuButtonTemplate"
            )
...
function CONDITIONERDROPDOWNMENU_AddButton(info, level)
    ...
    if (info.hasColorSwatch) then
        _G["ConditionerDropDownList" .. level .. "Button" .. index .. "ColorSwatch" .. "NormalTexture"]:SetVertexColor(
            info.r,
            info.g,
            info.b
        )
...
function ConditionerToggleDropDownMenu(
    level,
    value,
    dropDownFrame,
    anchorName,
    ...
    if (listFrame:IsShown() and (CONDITIONERDROPDOWNMENU_OPEN_MENU == tempFrame)) then
        ...
        if (level == 1) then
            ...
            listFrame:ClearAllPoints()
            if
                (strsub(button:GetParent():GetName(), 0, 12) == "ConditionerDropDownList" and
                    strlen(button:GetParent():GetName()) == 13)
             then
                anchorFrame = button
            else
